#!/usr/bin/env python3
"""
Create interface-side sulfur vacancies in the MoS2 layer of an h-AlN/MoS2 bilayer.

Default target for the uploaded structure:
    interface-side S atom types = {3, 6, 9, 12}

Default concentration:
    1.0% of the interface-side S atoms, not 1.0% of all S atoms.
...
Run:
    python create_VS_interface_MoS2.py
...
# In your uploaded bilayer, these S atoms are closest to the AlN layer.
S_INTERFACE_TYPES = {3, 6, 9, 12}

# These S atoms are on the free/outside MoS2 surface.
S_OUTER_TYPES = {1, 4, 7, 10}
S_ALL_TYPES = S_INTERFACE_TYPES | S_OUTER_TYPES
...
def write_summary(summary_file, input_file, output_file, id_file, atoms, selected, bounds, vacancy_fraction, seed, min_dist, edge_exclusion):
    ...
    target_total = sum(counts_before[t] for t in S_INTERFACE_TYPES)
    ...
    z_interface = [a["z"] for a in atoms if a["type"] in S_INTERFACE_TYPES]
    z_outer = [a["z"] for a in atoms if a["type"] in S_OUTER_TYPES]

    lines = []
    lines.append("Interface-side sulfur vacancy summary")
    ...
    lines.append("")
    lines.append(f"Target atom types: {sorted(S_INTERFACE_TYPES)}")
    lines.append(f"Outer S atom types not removed: {sorted(S_OUTER_TYPES)}")
    lines.append(f"Total interface-side S atoms before deletion: {target_total}")
    lines.append(f"Removed interface-side S atoms: {len(selected)}")
    lines.append(f"Actual vacancy fraction of interface-side S atoms: {actual_fraction:.8f}")
    lines.append(f"Actual vacancy percent of interface-side S atoms: {100.0 * actual_fraction:.6f}%")
    lines.append(f"Vacancy density using full xy cell area: {vacancy_density_cm2:.6e} cm^-2")
    lines.append("")
    lines.append(f"Mean z of interface-side S types: {sum(z_interface) / len(z_interface):.12f} Angstrom")
    lines.append(f"Mean z of outer-side S types: {sum(z_outer) / len(z_outer):.12f} Angstrom")
    lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="Create interface-side sulfur vacancies in MoS2 of h-AlN/MoS2.")
    parser.add_argument("--input", default="MoS2_AlN.lmp", help="Input LAMMPS data file")
    parser.add_argument("--output", default="bilayer_VSint_MoS2_1pct_seed01_unrelaxed.data", help="Output LAMMPS data file")
    parser.add_argument("--ids", default="ids_VSint_MoS2_1pct_seed01.txt", help="Output file containing removed original atom IDs")
    parser.add_argument("--summary", default="summary_VSint_MoS2_1pct_seed01.txt", help="Verification summary file")
    parser.add_argument("--fraction", type=float, default=0.01, help="Vacancy fraction of interface-side S atoms")
    ...
    all_target_atoms = [atom for atom in atoms if atom["type"] in S_INTERFACE_TYPES]
    ...
    print("Total interface-side S atoms:", len(all_target_atoms))
    print("Candidate interface-side S atoms after edge exclusion:", len(candidate_atoms))
    print("Removed interface-side S atoms:", len(selected))
    print("Actual interface-side S vacancy percent:", 100.0 * len(selected) / len(all_target_atoms))
